import re
import string
from pandora import COPY_reviews
import json

#Read in Yelp and Google Places data previously stored from API calls


ph_file = 'pandora/COPY_google_ph.json'
yelp = 'pandora/COPY_final_yelp_dic.json'
ot_file = 'pandora/COPY_ot_data.json'
...
with open('pandora/COPY_big_google.json', 'r') as f:
    data = f.read()
google_results = json.loads(data)

with open('pandora/COPY_sentiment_dic.json', 'r') as f:
    data = f.read()
